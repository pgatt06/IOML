...
The script \texttt{main\_merge.jl} replaces individual samples with same-class clusters that can no longer be split. Tables~\ref{tab:merge-results-small} and~\ref{tab:merge-results-large} keep only the best value of $\gamma$ at each depth. The univariate and multivariate columns therefore correspond to the best configuration found for each depth.
...
Dataset & $D$ & Univariate & Multivariate \\
\midrule
iris & 2 & \methodcell{20}{5}{1}{0.1}{0.0} & \methodcell{20}{1}{0}{0.5}{0.0} \\
iris & 3 & \methodcell{40}{0}{1}{2.7}{0.0} & \methodcell{100}{0}{0}{1.8}{0.0} \\
iris & 4 & \methodcell{40}{0}{1}{1.8}{0.0} & \methodcell{100}{0}{1}{14.0}{0.0} \\
\midrule
seeds & 2 & \methodcell{60}{12}{2}{2.2}{0.0} & \methodcell{40}{0}{0}{0.5}{0.0} \\
seeds & 3 & \methodcell{60}{5}{2}{180.0}{1.8} & \methodcell{40}{0}{1}{1.9}{0.0} \\
seeds & 4 & \methodcell{60}{4}{2}{180.1}{1.8} & \methodcell{80}{0}{0}{34.1}{0.0} \\
\midrule
wine & 2 & \methodcell{60}{5}{2}{2.1}{0.0} & \methodcell{100}{0}{1}{0.7}{0.0} \\
wine & 3 & \methodcell{80}{0}{2}{51.2}{0.0} & \methodcell{0}{0}{1}{0.1}{0.0} \\
wine & 4 & \methodcell{60}{0}{3}{44.2}{0.0} & \methodcell{80}{0}{0}{7.7}{0.0} \\
\bottomrule
\end{tabularx}
...
\midrule
wdbc & 2 & \methodcell{80}{18}{6}{180.1}{4.1} & \methodcell{20}{0}{6}{0.6}{0.0} \\
wdbc & 3 & \methodcell{80}{16}{4}{523.7}{3.9} & \methodcell{0}{0}{6}{1.6}{0.0} \\
wdbc & 4 & \methodcell{80}{29}{6}{180.3}{7.1} & \methodcell{40}{0}{4}{36.3}{0.0} \\
\bottomrule
\end{tabularx}
...
This grouped formulation remains credible in the univariate setting on the three smaller datasets. It reaches almost the same error levels as the exact univariate approach, often with lower times at depths 2 and 3. In the multivariate setting, it is actually strong on \texttt{iris}, \texttt{seeds}, \texttt{wine}, and \texttt{wdbc}: at the retained values of $\gamma$, the grouped trees reach zero training error on all four datasets and often very low test error in a few seconds. The clear exception is \texttt{banknote}, where no multivariate grouped run produced a feasible solution within \SI{180}{s}. Since the final grouped comparison is restricted to the five required datasets, the short exploratory grouped runs on \texttt{titanic} and \texttt{diabetes} are not discussed further here.

\subsection{Iterative Algorithms}

The script \texttt{main\_iterative\_algorithm.jl} is studied only in the univariate case. Tables~\ref{tab:iterative-results-small} and~\ref{tab:iterative-results-large} compare the best versions of $F_h^S$, $F_h^S$ with \emph{shifts}, and $F_e^S$. Here again, each cell reports train/test errors, total time, and the gap.
...
The clearest result in this section is the role of \emph{shifts}. The $F_h^S$ variant with \emph{shifts} is the most regular one: it reaches zero test error several times on \texttt{iris}, \texttt{seeds}, and \texttt{wine}, and also on \texttt{wdbc} at larger depths. The plain $F_h^S$ version remains useful but less stable. The exact version $F_e^S$ is rarely the best compromise; it is usually slower without a clear gain in test error. As in the grouped study, the short exploratory iterative runs on \texttt{titanic} and \texttt{diabetes} were not kept in the final comparison tables.

\section{Additional Work}

\subsection{Feature Augmentation}

\subsubsection{Algorithm for Feature Generation and Selection}
As suggested in the project guidelines, adding new features derived from simple mathematical combinations of existing ones can significantly simplify the classification task without losing the inherent interpretability of optimal classification trees. To achieve this, we avoided complex linear combinations (such as PCA) and implemented a systematic filtering algorithm:

\begin{enumerate}
    \item \textbf{Generation:} For every pair of existing numerical features $(X_a, X_b)$, we generated non-linear combinations: the sum ($X_a + X_b$), the absolute difference ($|X_a - X_b|$), the product ($X_a \times X_b$), the ratios ($X_a / X_b$), and absolute differences from the mean ($|X_a - \mu_a| + |X_b - \mu_b|$).
    \item \textbf{Evaluation via Correlation Matrix:} We evaluated the predictive power of each newly generated feature by computing the absolute Pearson correlation score with respect to the target variable $Y$.
    \item \textbf{Selection:} To prevent combinatorial explosion and avoid overloading the exact formulation $F$, the algorithm ranks the generated features by their correlation score and only appends the top $k=3$ features to each dataset.
\end{enumerate}

\subsubsection{Results of Feature Generation}

Applying this algorithm to the five datasets yielded the following new features. As shown in the table below, the combined features systematically exhibit a higher linear correlation with the target class than any of the original isolated variables.

\begin{table}[htbp]
\centering
\caption{Feature Engineering Results: Original vs. Top 3 Generated Features}
\resizebox{\textwidth}{!}{
\begin{tabular}{lcccccc}
\hline
\textbf{Dataset} & \textbf{Original Size} & \textbf{Best Original Feature} & \textbf{Top 1 Generated} & \textbf{Top 2 Generated} & \textbf{Top 3 Generated} & \textbf{New Size} \\
\hline
\multirow{2}{*}{\textbf{Iris}} & \multirow{2}{*}{150 $\times$ 4} 
 & F4 & F3 + F4 & F3 $\times$ F4 & F1 + F4 & \multirow{2}{*}{150 $\times$ 7} \\
 & & $(0.956)$ & \textbf{(0.962)} & (0.942) & (0.925) & \\
\hline
\multirow{2}{*}{\textbf{Seeds}} & \multirow{2}{*}{210 $\times$ 6} 
 & F5 & $|F1 - \mu_1| + |F4 - \mu_4|$ & $|F2 - \mu_2| + |F3 - \mu_3|$ & $|F1 - \mu_1| + |F2 - \mu_2|$ & \multirow{2}{*}{210 $\times$ 9} \\
 & & $(0.577)$ & (0.565) & (0.554) & (0.546) & \\
\hline
\multirow{2}{*}{\textbf{Wine}} & \multirow{2}{*}{178 $\times$ 13} 
 & F7 & F12 + F13 & F7 + F12 & F7 + F13 & \multirow{2}{*}{178 $\times$ 16} \\
 & & $(0.847)$ & \textbf{(0.883)} & \textbf{(0.861)} & \textbf{(0.853)} & \\
\hline
\multirow{2}{*}{\textbf{Titanic}} & \multirow{2}{*}{891 $\times$ 6} 
 & F2 & F2 + F6 & $|F2 - F5|$ & $|F2 - F4|$ & \multirow{2}{*}{891 $\times$ 9} \\
 & & $(0.543)$ & \textbf{(0.564)} & \textbf{(0.558)} & (0.540) & \\
\hline
\multirow{2}{*}{\textbf{Diabetes}} & \multirow{2}{*}{6000 $\times$ 17} 
 & F17 & $|F9 - F17|$ & $|F11 - F17|$ & F16 $\times$ F17 & \multirow{2}{*}{6000 $\times$ 20} \\
 & & $(0.583)$ & \textbf{(0.684)} & \textbf{(0.679)} & \textbf{(0.639)} & \\
\hline
\end{tabular}
}
\end{table}
\textbf{Feature Correlation Analysis:}
The simple mathematical combinations successfully extracted stronger predictive signals. For example, on the \textit{Diabetes Risk} dataset, the best original feature (F17) had a correlation of $0.583$ with the target. By combining it with F9 using an absolute difference ($|F9 - F17|$), the correlation jumped significantly to $\textbf{0.684}$. Similarly, on the \textit{Wine} dataset, adding F12 and F13 created a highly discriminative feature reaching a correlation of $0.883$. 
\textit{(Note: On the Seeds dataset, the original feature F5 remained marginally more correlated than the generated distance-to-mean features, but these new features still provide orthogonal splits that may benefit the tree).}

\subsubsection{Impact on Classification Performance}
We finally obtained these results of classification at depth 2:
...
\midrule
iris & 1 & baseline & 0 / 0 & 120.0 & 5 & 12.5 & 1606 \\
iris & 1 & all equalities & 237 / 4 & 120.0 & 5 & 8.1 & 317 \\
iris & 1 & cutting plane & 186 / 0 & 120.0 & 5 & 10.0 & 433 \\
\midrule
seeds & 1 & baseline & 0 / 0 & 168.0 & 16 & 28.7 & 5409 \\
seeds & 1 & all equalities & 60 / 0 & 168.0 & 16 & 30.1 & 5406 \\
seeds & 1 & cutting plane & 44 / 0 & 168.0 & 16 & 30.7 & 5668 \\
\midrule
wine & 0 & baseline & 0 / 0 & 142.0 & 21 & 15.3 & 139 \\
wine & 0 & all equalities & 1421 / 0 & 142.0 & 21 & 8.8 & 0 \\
wine & 0 & cutting plane & 1028 / 0 & 142.0 & 21 & 9.3 & 0 \\
\bottomrule
\end{tabularx}
...
\midrule
iris & none & 0 / 0 & 0 / 0 & 5 & 8.3 & 1223 \\
iris & 2 & 0 / 0 & 0 / 0 & 5 & 15.7 & 2170 \\
iris & 1 & 237 / 4 & 186 / 0 & 5 & 8.3 & 433 \\
iris & 0 & 1333 / 0 & 741 / 0 & 22 & 2.9 & 0 \\
\midrule
seeds & none & 0 / 0 & 0 / 0 & 11 & 30.5 & 4921 \\
seeds & 2 & 0 / 0 & 0 / 0 & 11 & 30.4 & 4655 \\
seeds & 1 & 60 / 0 & 44 / 0 & 16 & 24.5 & 5663 \\
seeds & 0 & 2866 / 0 & 556 / 0 & 25 & 4.5 & 0 \\
\midrule
wine & none & 0 / 0 & 0 / 0 & 5 & 30.4 & 3593 \\
wine & 2 & 0 / 0 & 0 / 0 & 8 & 30.5 & 3057 \\
wine & 1 & 0 / 0 & 0 / 0 & 7 & 20.8 & 2270 \\
wine & 0 & 1421 / 0 & 1028 / 0 & 21 & 8.8 & 0 \\
\bottomrule
\end{tabularx}
...
The grouped formulation $F_U$ is useful as a univariate approximation on medium-sized datasets, and its multivariate version is also competitive on the five retained datasets except \texttt{banknote}. In the iterative part, the strongest variant remains $F_h^S$ with \emph{shifts}: it is the one that most often gives the best compromise in test error on the retained runs.
